...
class Loader {
  ...
  fork(options = {}) {
    let { cmd, args = [] }  = options
      , { log }             = this
      , env                 = _.defaults({ NODE_ENV : 'production' }, process.env)
      , exec_options        = { env, silent : false, async : true, cwd }
      ;

    let proc = shell.exec([cmd, ...args].join(' '), exec_options);
